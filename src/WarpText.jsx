...
const measureLine = (context, line, spacing) => {
  const chars = Array.from(line);
  return chars.reduce((width, char) => width + context.measureText(char).width, 0) + Math.max(0, chars.length - 1) * spacing;
};

const drawLine = (context, line, x, y, spacing) => {
  const chars = Array.from(line);
...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  if (!Number.isFinite(lineHeight)) lineHeight = fontSize * (typeof props.lineHeight === "number" ? props.lineHeight : 0.92);
  probe.remove();
  ...
  const fit = Math.min(
    1,
    (width * 0.86) / Math.max(...lines.map(line => measureLine(context, line, letterSpacing)), 1),
    (height * 0.78) / Math.max(lineHeight * lines.length, 1)
  );
  if (fit < 1) {
    fontSize *= fit;
    letterSpacing *= fit;
...
const syncUniforms = (program, props) => {
  ...
  program.uniforms.uWarpScale.value = props.warpScale;
  program.uniforms.uSpeed.value = props.speed;
  program.uniforms.uPointerInfluence.value = props.pointerInfluence;
  program.uniforms.uPointerStrength.value = props.pointerStrength;
  program.uniforms.uRefraction.value = props.refraction;
  program.uniforms.uRipple.value = props.ripple ? 1 : 0;
};
...
export default function WarpText({
  ...
  useEffect(() => {
    ...
    const loop = now => {
      ...
      const targetX = pointer.activeTarget ? pointer.tx : 0.5 + Math.sin(elapsed * 0.33) * 0.12;
      const targetY = pointer.activeTarget ? pointer.ty : 0.5 + Math.cos(elapsed * 0.27) * 0.1;
